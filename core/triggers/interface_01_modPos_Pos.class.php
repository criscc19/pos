...
class InterfacePos
{
	...
	public function run_trigger($action,$object,$user,$langs,$conf)
    {
    	$errors=0;
		// Actions
		if ($conf->rewards->enabled) return 0; 
    	/*
...
if($remaintopay >= (float)($conf->global->POS_MAX_NEGATIVE_MONTO) && $remaintopay<0){
	$arr= array($key=>$remaintopay);
	$positivo = new Paiement($db);
		$positivo->datepaye     = date('Y-m-d');
		$positivo->amounts      = $arr;   // Array with all payments dispatching with invoice id
		$positivo->multicurrency_amounts = array();   // Array with all payments dispatching
		$positivo->paiementid   = dol_getIdFromCode($db,'DIF','c_paiement','code','id',1);
		$positivo->num_paiement = $numpago;
		$positivo->note         = 'PAGO POR DIFERENCIAL NEGATIVO';
		
		$res = $positivo->create($user, 1);

		//if ($a['type'] == Facture::TYPE_CREDIT_NOTE) $label='(CustomerInvoicePaymentBack)';  // Refund of a credit note
		if($res > 0){
		$resp = $positivo->addPaymentToBank($user,'payment','Pago por retencion',$conf->global->POS_BANK_DIFERENCIAL,'Direfencial','Direfencial');
		}
	}
	
	if($remaintopay <= (float)($conf->global->POS_MAX_POSITIVE_MONTO) && $remaintopay > 0){
		$arr= array($key=>$remaintopay);
		$positivo = new Paiement($db);
			$positivo->datepaye     = date('Y-m-d');
			$positivo->amounts      = $arr;   // Array with all payments dispatching with invoice id
			...
			$positivo->paiementid   = dol_getIdFromCode($db,'DIF','c_paiement','code','id',1);
			$positivo->num_paiement = $numpago;
			$positivo->note         = 'PAGO POR DIFERENCIAL POSITIVO';
...
//$invoice->set_paid($user, $close_code = '', $close_note = 'PAGADA POR DIFERENCIAL');
//fin de pasando factura a validado
 */
	...
	public function setPoints ($facid,$amount,$isdelete=0)
    {
		...
		{
			if($conf->global->REWARDS_RATIO > 0){
