...
dol_include_once('/pos/lib/rewards.lib.php');
 ...
 */
// conditions rewards
if ($action === 'setconditions' && $user->rights->pos->ad_rewards)
{
...
if ($socid)
{
	...
	print '<td>';
	if (($action != 'editconditions') && $user->rights->pos->ad_rewards) print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editconditions&amp;socid='.$socid.'">'.img_edit($langs->trans('SetConditions'),1).'</a></td>';
	print '</tr></table>';
	print '</td><td colspan="3">';
	if ($action == 'editconditions' && $user->rights->pos->ad_rewards)
	{
