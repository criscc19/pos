...
$m = $_POST['m']; //moneda
$cantidad = (float)$_POST['c']; //cantidad
...
if($m == 'CRC'){
  ...
  if($pro->multiprices_base_type[$nivel] == 'HT'){
    $precio_min = $pro->multiprices_min[$nivel];
  }else{
   $precio_min = $pro->multiprices_min_ttc[$nivel];    
  }  
}
  else{
 ...
 $imgs = $images->productImage(0,$pro->id);
  ...
  if($m == 'CRC'){
    $result[$f]['precio']=price($pro->multiprices[$nivel]);
    if($pro->multiprices_base_type[$nivel] == 'HT'){
      $result[$f]['precio_min']= price($pro->multiprices_min[$nivel]);
    }else{
      $result[$f]['precio_min']= price($pro->multiprices_min_ttc[$nivel]);    
    }   
  }else{
