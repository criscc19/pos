...
var TicketLine = jQuery.Class({
	...
	getHtml:function(){
		...
		if(this.diff_price == 0){
			return '<tr data-id="'+this.idProduct+'" id="ticketLine'+this.idProduct+'" onclick="_TPV.ticket.showTicketOptions('+this.idProduct+')"><td class="idCol">'+this.idProduct+'</td><td class="description">'+this.label+'</td><td class="discount">'+this.discount+'%</td><td class="price">'+displayPrice(this.total/this.cant)+'</td><td class="cant">'+this.cant+'</td><td class="total">'+displayPrice(this.total)+'</td><td class="colActions"><a class="action edit" onclick="_TPV.ticket.editTicketLine('+this.idProduct+');"></a><a class="action delete" onclick="_TPV.ticket.deleteLine('+this.idProduct+');"></a><a class="action info" onclick="'+hide+'"></a><a class="action close" onclick="_TPV.ticket.hideTicketOptions('+this.idProduct+')"></a></td></tr>';
		}
		else{
...
var TPV = jQuery.Class({
	...
	setButtonEvents:function()
	{
		...
		$('#btnChangeActividad').click(function(){
			_TPV.changeActividad();
		});	
    ...
    changeActividad: function()
	{
    	$('#idChangeActividad').dialog({ modal: true });
		$('#idChangeActividad').dialog({height:200,width: 600});
	},
	...
	showInfo: function(error)
	{
		...
		setTimeout(function(){$('#idPanelInfo').dialog("close")},3000);
	},
	showError: function(error)
	{
		...
		setTimeout(function(){$('#idPanelError').dialog("close")},6000);
	},
