		...
		if ($result)
		{
			...
			if (! empty($object->lines))
			{
				...
				foreach ($object->lines as $line)
				{
					...
					echo ('<tr><td align="left">'.$line->libelle.'</td><td align="right">'.$line->qty." * ".price($line->subprice,"","","","",2).'</td><td align="right">'.$line->remise_percent.'%</td><td class="total">'.price($line->total_ht,"","","","",2).' '.$langs->trans(currency_name($conf->currency)).'</td></tr>'."\n");
					$subtotal+=$totalline;
...
<table class="totaux">
	<?php
		...
		if($object->total_localtax1!=0){
			echo '<tr><th nowrap="nowrap">'.$langs->transcountrynoentities("TotalLT1",$mysoc->country_code).'</th><td nowrap="nowrap">'.price($object->total_localtax1)." ".$langs->trans(currency_name($conf->currency))."</td></tr>\n";
		}
		if($object->total_localtax2!=0){
			echo '<tr><th nowrap="nowrap">'.$langs->transcountrynoentities("TotalLT2",$mysoc->country_code).'</th><td nowrap="nowrap">'.price($object->total_localtax2)." ".$langs->trans(currency_name($conf->currency))."</td></tr>\n";
		}
