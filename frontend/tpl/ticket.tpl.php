		...
		if ($result)
		{
			...
			if (! empty($object->lines))
			{
				...
				foreach ($object->lines as $line)
				{
					...
					echo ('<tr><td align="left">'.$line->libelle.'</td><td align="right">'.$line->qty." * ".price($line->subprice,"","","","",2).'</td><td align="right">'.$line->remise_percent.'%</td><td class="total">'.price($totalline,"","","","",2).' '.$langs->trans(currency_name($conf->currency)).'</td></tr>'."\n");
					$subtotal+=$totalline;
