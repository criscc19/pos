...
$sql = 'SELECT ';
$sql.= ' f.rowid, f.ref, f.fk_cash, f.fk_user, f.amount_teor, f.amount_real,  f.amount_diff,f.date_close,';
		...
		//$datelimit=$db->jdate($objp->datelimite);
			...
			if($objp->type_control){
				if($conf->global->POS_FACTURE == 1){
					print '<a href="listefac.php?closeid='.$objp->rowid.'&terminalid='.$objp->fk_cash.'&viewstatut=2">'.$objp->ref.'</a>';
				}
				else{	
					print '<a href="liste.php?closeid='.$objp->rowid.'&terminalid='.$objp->fk_cash.'&viewstatut=2">'.$objp->ref.'</a>';
				}
			}
			else {
				print $objp->ref;
			...
			print '<td align="left" nowrap="nowrap">';
 			if($objp->date_close == ''){
            print img_picto($langs->trans("NotInUse"),'statut4').' Caja Abierta';
			}else{
			print img_picto($langs->trans("Cerrada"),'statut8'). 'Cerrada';	
			}
