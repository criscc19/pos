...
if ($conf->rewards->enabled) {dol_include_once('/rewards/class/rewards.class.php');}
if (!$conf->rewards->enabled) {dol_include_once('/pos/backend/class/rewards.class.php');}
dol_include_once('/pos/backend/class/facturesim.class.php');
if ($conf->rewards->enabled) {dol_include_once (DOL_DOCUMENT_ROOT.'/rewards/class/rewards.class.php');}
if (!$conf->rewards->enabled) {dol_include_once (DOL_DOCUMENT_ROOT.'/pos/backend/class/rewards.class.php');}
...
class POS extends CommonObject
{
 	...
 	*  @return      array					Product info
 	*/
	public static function SearchProduct($idSearch,$stock=false, $warehouse,$mode=0, $ticketstate=0)
	{
		global $db, $conf;
		...
		if($mode>=0){
			if ($stock)
			{
				...
				$sql .=" , ep.ref as warehouse, ep.rowid as warehouseId";
		...
		$sql= "SELECT distinct p.rowid, p.ref, p.label , w.reel as stock, w.fk_entrepot as warehouseId, e.label as warehouse ";
			...
			if($mode == -5){//best sell
				...
				$sql.="	(select w.reel from ".MAIN_DB_PREFIX."product_stock w left join ".MAIN_DB_PREFIX."entrepot e on w.fk_entrepot = e.rowid";
				$sql.=" where w.fk_product = pr.rowid and e.rowid=ep.rowid) as stock, ep.ref as warehouse, ep.rowid as warehouseId";
    		...
    		foreach ( $lines as $line )
    		{
    			if(sizeof($line)>0)
    			{
    				if ($line['idProduct']>0)
    				{
    					...
    					$res= $object->addline($line['description'], $line['price'], $qty,  $line['tva_tx'], $txlocaltax1, $txlocaltax2, $line['idProduct'], $line['discount'], $date_start, $date_end, $ventil, $info_bits, $fk_remise_except,$line['price_base_type'],$line['price_ttc'],$line['fk_product_type'], $rang, $special_code, $origin, $origin_id, $fk_parent_line, $fk_fournprice, $pa_ht, $label, $array_options, $situation_percent, $fk_prev_id, $fk_unit, $pu_ht_devise);
	...
	private function addPayment($aryTicket)
	{
        ...
        else if($aryTicket['payment_type']==$cash->fk_modepaybank)
        {
        	$bankaccountid = $cash->fk_paybank ;
        	$amount = $aryTicket['customerpay'];
        }
        else{
        	$bankaccountid = $cash->fk_paybank_extra ;
        	$amount = $aryTicket['customerpay'];
        }
	...
	public static function fillMailTicketBody($id)
	{
		...
		if(! empty($subtotaltva)){
			foreach($subtotaltva as $tvakey => $tvaval){
				if($tvakey > 0)
					$message .= $langs->transnoentities("TotalVAT").' '.round($tvakey)."%:\t".price($tvaval)." ".$langs->trans(currency_name($conf->currency))."\n";
			}
		}
		
		
		if($ticket->total_localtax1!=0){
			$message .= $langs->transcountrynoentities("TotalLT1",$mysoc->country_code).":\t".price($ticket->total_localtax1)." ".$langs->trans(currency_name($conf->currency))."\n";
		}
		if($ticket->total_localtax2!=0){
			$message .= $langs->transcountrynoentities("TotalLT2",$mysoc->country_code).":\t".price($ticket->total_localtax2)." ".$langs->trans(currency_name($conf->currency))."\n";
		}
	...
	public static function fillMailFactureBody($id)
	{
		...
		if($facture->total_localtax1!=0){
			$message .= $langs->transcountrynoentities("TotalLT1",$mysoc->country_code).":\t".price($facture->total_localtax1)." ".$langs->trans(currency_name($conf->currency))."\n";
		}
		if($facture->total_localtax2!=0){
			$message .= $langs->transcountrynoentities("TotalLT2",$mysoc->country_code).":\t".price($facture->total_localtax2)." ".$langs->trans(currency_name($conf->currency))."\n";
		}
	...
	public static function FillMailCloseCashBody($id,$terminal)
	{
		...
		if(! empty($subtotalcashht)) $message .= $langs->transnoentities("TotalHT")."\t\t\t"." ".price($subtotalcashht)." ".$langs->trans(currency_name($conf->currency))."\n";
		...
		if($subtotalcashlt1)
			$message .= $langs->transcountrynoentities("TotalLT1",$mysoc->country_code)."\t\t\t"." ".price($subtotalcashlt1)." ".$langs->trans(currency_name($conf->currency))."\n";
		if($subtotalcashlt2)
			$message .= $langs->transcountrynoentities("TotalLT2",$mysoc->country_code)."\t\t\t"." ".price($subtotalcashlt2)." ".$langs->trans(currency_name($conf->currency))."\n";
		...
		$sql = "select ticketnumber, total_ttc, type";
		$sql .=" from ".MAIN_DB_PREFIX."pos_ticket";
		$sql .=" where fk_control = ".$id." and fk_cash=".$terminal." and (fk_mode_reglement=".$cash->fk_modepaybank." or fk_mode_reglement=".$cash->fk_modepaybank_extra.")";
		
		$sql .= " union select f.facnumber, f.total_ttc, f.type";
		$sql .=" from ".MAIN_DB_PREFIX."pos_facture as pf,".MAIN_DB_PREFIX."facture as f ";
		$sql .=" where pf.fk_control_cash = ".$id." and pf.fk_cash=".$terminal." and (f.fk_mode_reglement=".$cash->fk_modepaybank." or fk_mode_reglement=".$cash->fk_modepaybank_extra.") and pf.fk_facture = f.rowid and f.fk_statut > 0";
		$result=$db->query($sql);
		...
		$sql = "select t.ticketnumber, t.type, l.total_ht, l.tva_tx, l.total_tva, l.total_localtax1, l.total_localtax2";
		$sql .=" from ".MAIN_DB_PREFIX."pos_ticket as t left join ".MAIN_DB_PREFIX."pos_ticketdet as l on l.fk_ticket= t.rowid";
		$sql .=" where t.fk_control = ".$id." and t.fk_cash=".$terminal." and (t.fk_mode_reglement=".$cash->fk_modepaybank." or t.fk_mode_reglement=".$cash->fk_modepaybank_extra.") and t.fk_statut > 0";
		
		$sql .= " union select f.facnumber, f.type, fd.total_ht, fd.tva_tx, fd.total_tva, fd.total_localtax1, fd.total_localtax2";
		$sql .=" from ".MAIN_DB_PREFIX."pos_facture as pf,".MAIN_DB_PREFIX."facture as f left join ".MAIN_DB_PREFIX."facturedet as fd on fd.fk_facture= f.rowid";
		$sql .=" where pf.fk_control_cash = ".$id." and pf.fk_cash=".$terminal." and (f.fk_mode_reglement=".$cash->fk_modepaybank. " or f.fk_mode_reglement=".$cash->fk_modepaybank_extra. ") and pf.fk_facture = f.rowid and f.fk_statut > 0";
		...
		if(! empty($subtotalcardht))$message .= $langs->transnoentities("TotalHT")."\t\t\t"." ".price($subtotalcardht)." ".$langs->trans(currency_name($conf->currency))."\n";
