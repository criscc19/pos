...
class Cash extends CommonObject
{
    ...
    function create($user)
    {
        ...
        $sql.= ", '".$this->name."'";
        $sql.= ", ".$this->tactil;
        $sql.= ", ".($this->barcode?$this->barcode:"0");
        ...
        $sql.= ", '".$this->db->idate($now)."'";
        $sql.= ", '".($this->bank_active?$this->bank_active:"null")."'";        
        $sql.= ")";
    ...
    function fetch($rowid, $ref='')
    {
        ...
        if ($result)
        {
            if ($this->db->num_rows($result))
            {
                ...
                $this->bank_active		= $obj->bank_active;    
                return 1;
            }
            else
            {
    ...
    function update($user)
    {
        ...
        if (isset($this->fk_warehouse)) $this->fk_warehouse=trim($this->fk_warehouse);
 	    ...
 	    if (isset($this->bank_active)) $this->bank_active=trim($this->bank_active); 	        
        // Check parameters
        $now=dol_now();
        ...
        $sql.= " barcode=".($this->barcode?$this->barcode:"0").",";
        ...
        $sql.= " bank_active='".(isset($this->bank_active)?$this->bank_active:"null")."',";        
...
class ControlCash extends CommonObject
{
	...
	var $amount_reel;
    var $amount_diff;   
	var $multicurrency_amount_teoric;
	var $multicurrency_amount_reel;
    var $multicurrency_amount_diff;    
	var $amount_nextday;
    ...
    function Create($data)
    {
        ...
        $sql.= ", fk_user";
        $sql.= ", amount_teor";   
        $sql.= ", multicurrency_amount_teor";                
        $sql.= ", amount_real";
        $sql.= ", multicurrency_amount_real";        
        $sql.= ", type_control";
        $sql.= ", date_c";
        $sql.= ", comment";        
        $sql.= ", date_open";
        ...
        $sql.= ", ".$data['multicurrency_amount_teoric'];         
        $sql.= ", ".$data['amount_reel'];
        $sql.= ", ".$data['multicurrency_amount_reel'];        
		$sql.= ", ".$data['type_control'];
        $sql.= ", '".$db->idate($now)."'";
        $sql.= ", '".$this->comment."'";        
        $sql.= ", '".$db->idate($now)."'";        
        $sql.= ", NULL";
        $sql.= ",' ".$data['fk_responsable']."'";
        $sql.= ", '".$data['fk_cierre']."'";                              
        $sql.= ")";
...
function fetch($id){
global $db;
$sq = 'SELECT rowid,ref,fk_user,date_open,amount_real,amount_teor,amount_diff,multicurrency_amount_real,multicurrency_amount_teor,
multicurrency_amount_diff,fk_responsable,fk_cierre FROM llx_pos_control_cash WHERE rowid = '.$id.'';
$sql = $db->query($sq);
while($obj = $db->fetch_object($sql)){
$this->id = $obj->rowid;
$this->ref = $obj->ref;
$this->user_open = $obj->fk_user;
$this->date_open = $obj->date_open;
$this->amount_reel = $obj->amount_real;
$this->amount_teoric = $obj->amount_teor;
$this->amount_diff = $obj->amount_diff;
$this->multicurrency_amount_real = $obj->multicurrency_amount_real;
$this->amount_teoric = $obj->multicurrency_amount_teor;
$this->amount_diff = $obj->multicurrency_amount_diff;
$this->fk_responsable = $obj->fk_responsable;
$this->fk_cierre = $obj->fk_cierre;
}
}
	...
	function closeControlCash($id,$user_close,$date_close)
	{
		global $db;
        $sql = 'UPDATE llx_pos_control_cash';
        $sql .= ' SET date_close = "'.$date_close.'",';
        $sql .= ' user_close = "'.$user_close.'"';         
        $sql .= ' WHERE rowid = '.$id.'';	
		$res = $db->query($sql);
		return $res;		
	}    
	...
	function get_cash_id_open($user_id)
	{
		global $db;
        $sq = 'SELECT rowid FROM llx_pos_control_cash 
        WHERE fk_cash = '.$this->terminal.' AND fk_user='.$user_id.' AND type_control=1 AND date_close IS NULL
