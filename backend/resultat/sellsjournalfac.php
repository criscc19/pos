...
foreach ($tabfac as $key => $val)
{
	...
	foreach ($tabltx1[$key] as $k => $mt)
	{
		if ($mt)
		{
			...
			print "<td>".dol_print_date($facturestatic->date_validation,"day")."</td>";
			print "<td>".$facturestatic->getNomUrl(1)."</td>";
			print "<td>".$k."</td><td>".$langs->transcountrynoentities("TotalLT1",$mysoc->country_code)."</td><td align='right'>".($mt<0?price(-$mt):'')."</td><td align='right'>".($mt>=0?price($mt):'')."</td></tr>";
			$totalltx1+=$mt;
		}
	}
	...
	foreach ($tabltx2[$key] as $k => $mt)
	{
		if ($mt)
		{
			...
			print "<td>".$facturestatic->getNomUrl(1)."</td>";
			//print "<td>".$invoicestatic->getNomUrl(1)."</td>";
			print "<td>".$k."</td><td>".$langs->transcountrynoentities("TotalLT2",$mysoc->country_code)."</td><td align='right'>".($mt<0?price(-$mt):'')."</td><td align='right'>".($mt>=0?price($mt):'')."</td></tr>";
			$totalltx2-=$mt;
		}
	}
